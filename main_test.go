...
func TestRateMeterFollowsASlowdown(t *testing.T) {
	...
	fast := m.observe(flatten.Progress{Done: 1, Bytes: 1_000_000_000, Elapsed: 10 * time.Second})
	...
	for i := 0; i < 5; i++ {
		...
		slow = m.observe(flatten.Progress{
			Done:    2 + i,
			Bytes:   bytes,
...
func TestRateMeterIgnoresBursts(t *testing.T) {
	m := &rateMeter{}
	first := m.observe(flatten.Progress{Done: 1, Bytes: 10_000_000, Elapsed: time.Second})

	// A second sample 1ms later, below the resample interval.
	got := m.observe(flatten.Progress{Done: 2, Bytes: 10_000_100, Elapsed: time.Second + time.Millisecond})
	if got != first {
...
func TestProgressLineShowsThroughput(t *testing.T) {
	...
	line := progressLine(p, 58_300_000)
	...
	done := progressLine(flatten.Progress{Done: 7, Total: 7, Bytes: 100, Elapsed: time.Second}, 100)
